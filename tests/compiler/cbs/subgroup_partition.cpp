...
int main() {
    ...
    queue.submit([&](cl::sycl::handler &cgh) {
      ...
      cgh.parallel_for<class dynamic_local_memory_reduction>(
          cl::sycl::nd_range<1>{global_size, local_size}, [=](cl::sycl::nd_item<1> item) noexcept {
            ...
            auto i = cl::sycl::partition(sg, val, [](int a) { return a <= 2 and a != 0; });
            if (item.get_global_linear_id() == 31) {
              acc[item.get_global_id()] = i;
            }
          });
    });
  }
  ...
  std::sort(host_buf.begin(), host_buf.begin() + 2);
